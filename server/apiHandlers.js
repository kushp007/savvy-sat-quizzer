...
export async function healthHandler(_req, res) {
  return res.json({ ok: true, hasOpenAiKey: Boolean(process.env.OPENAI_API_KEY) });
}
...
export async function generateQuestionsHandler(req, res) {
  ...
  const batchSize = Math.min(20, Math.max(5, Number(body?.batchSize) || 15));
  const recent = Array.isArray(body?.recentQuestionStems) ? body.recentQuestionStems : [];
  const recentBlock =
    recent.length > 0
...
- Keep stems concise (SAT-style).`;
  ...
  try {
    ...
    console.error(`[sat-quiz-api] OpenAI generation failed: ${safeError.message}`, {
...
export async function generateFeedbackHandler(req, res) {
  const parsedReq = FeedbackRequestSchema.safeParse(getJsonBody(req));
